...
import { getGravityForm } from "../lib/wordpress";
...
export default function GravityForm({ formId = 1 }) {
  ...
  useEffect(() => {
    getGravityForm(formId).then(setForm);
  }, [formId]);
                      ...
                      {field.choices.map((choice, idx) => (
                        <label key={idx} className="inline-flex items-center">
                          <input
                            ...
                            value={choice.value}
                            checked={
                              Array.isArray(fields[`input_${field.id}`])
                                ? fields[`input_${field.id}`].includes(
                                    choice.value
                                  )
                                : false
                            }
                      ...
                      {field.choices.map((choice, idx) => (
                        <label key={idx} className="inline-flex items-center">
                          <input
                            ...
                            value={choice.value}
                            checked={
                              fields[`input_${field.id}`] === choice.value
                            }
                            onChange={(e) =>
                              handleChange(e, `input_${field.id}`)
                            }
                            required={field.isRequired}
                          />
                          {choice.text}
